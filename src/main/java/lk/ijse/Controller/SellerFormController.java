...
public class SellerFormController implements Initializable {
    ...
    @FXML
    private TableView<SellerTM> selltable;
    ...
    ObservableList<SellerTM> obList = FXCollections.observableArrayList();

    @FXML
    void buylblClicked(MouseEvent event) throws IOException {
        new Stageset().getPane("/View/Buyer_form.fxml",sellerPane);
    }
    ...
    void supPressed(ActionEvent event) {
        try {
            ...
            Seller seller = SellerModel.search(code);
            if(!(seller ==null)){
        ...
        if(!(supidtxt.getText().equals(""))){
            ...
            if(saveBtn.getText().equals("Update")) {
                ...
                try {
                    ...
                    boolean isSaved = SellerModel.update(seller);
                    if (isSaved) {
                        supidtxt.setEditable(true);
                        saveBtn.setText("Save");
                        refresh();
    ...
    private void setCellValueFactory() {
        ...
        Callback<TableColumn<SellerTM,String>, TableCell<SellerTM,String>> cellfactory = employeeTMStringTableColumn -> {
            final TableCell<SellerTM,String> cell = new TableCell<SellerTM,String>(){
                ...
                protected void updateItem(String item, boolean empty) {
                    ...
                    if(empty){
                        ...
                        ebtn.setOnAction(event -> {
                            SellerTM tm = getTableView().getItems().get(getIndex());
                            try {
                                altbtn();
                                clear();
                                refresh();
    ...
    private void altbtn() throws SQLException {
        ...
        if (result.orElse(no) == yes) {
            ...
            if(index>=0){
                SellerTM em = selltable.getSelectionModel().getSelectedItem();
                boolean isOkay = SellerModel.delete(em.getId());
                if(isOkay){
    ...
    private void getAll() {
        ...
        try {
            List<Seller> cusList = SellerModel.getAll();

            for (Seller seller : cusList) {
                obList.add(new SellerTM(
                        seller.getId(),
                        seller.getName(),
                        seller.getAddress(),
                        seller.getDate(),
    ...
    @FXML
    void timePressed(KeyEvent event) {
